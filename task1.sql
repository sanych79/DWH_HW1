...
SELECT
  m.music_key::varchar as code, -- исправил m.id на music_key
  m.album as name,
...
INSERT INTO dim.product (code, name, artist, product_type, product_category, unit_price, unit_cost, status, effective_ts, expire_ts, is_current)
SELECT
  f.film_key::varchar as code,-- используем ключ а не ИД
  f.title  as name,
  'Неизвестно' as artist,
  ...
  f.is_current
FROM nds.films f
join nds.films_genre fg on fg.id = f.genre_id 
;
--- КОНЕЦ Добавление данных по фильмам
...
-- Модифицировал вставку факта относительно ключей музыки и фильмов, взял деньги относительно даты транзакции. Не понимаю почему заполнение таблицы так долго идет

INSERT INTO fact.sale_item (date_key, customer_key, product_key, store_key, dt, transaction_id, line_number, quantity, unit_price, unit_cost, sales_value, sales_cost, margin)
select 
	 to_char(dt, 'YYYYMMDD')::int as date_key,
     customer_id,
     p.id,
     si.store_id as store_key,--Добавил ИД магазина
...
JOIN dim.product p ON (p.code::int = m.music_key or p.code::int = f.film_key)  and si.dt  BETWEEN p.effective_ts and p.expire_ts  
